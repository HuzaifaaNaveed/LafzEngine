...
class UrduTransliterator:
    def __init__(self, model_path: str, csv_path: str):
        ...
        ner_tokenizer = AutoTokenizer.from_pretrained("davlan/xlm-roberta-base-wikiann-ner")
        ner_model = AutoModelForTokenClassification.from_pretrained("davlan/xlm-roberta-base-wikiann-ner")
        self.ner_pipeline = pipeline("ner", model=ner_model, tokenizer=ner_tokenizer, aggregation_strategy="simple")
    ...
    def transliterate_with_ner(self, urdu_sentence: str) -> Tuple[str, Dict]:
        ner_results = self.ner_pipeline(urdu_sentence)
        
        if not ner_results:
            return self.transliterate_with_lookup(urdu_sentence)
        ...
        for entity in ner_results:
            ...
            entity_positions.append((start, end, entity_text))
        ...
        for start, end, entity_text in entity_positions:
            if current_pos < start:
                non_entity_part = urdu_sentence[current_pos:start]
                non_entity_parts.append(non_entity_part)
            
            non_entity_parts.append("")
            entity_list.append((len(non_entity_parts)-1, entity_text))
            current_pos = end
        
        if current_pos < len(urdu_sentence):
            non_entity_parts.append(urdu_sentence[current_pos:])
        
        transliterated_parts = []
        for part in non_entity_parts:
            if part:
                part_transliterated = self.transliterate_batch([part])[0]
                transliterated_parts.append(part_transliterated)
            else:
                transliterated_parts.append("")
        
        entity_transliterations = {}
        display_map = {}
        for idx, entity_text in entity_list:
            entity_transliterated, entity_details = self.transliterate_with_lookup(entity_text)
            entity_transliterations[idx] = entity_transliterated
            display_map.update(entity_details)
        
        for idx, entity_transliterated in entity_transliterations.items():
            transliterated_parts[idx] = entity_transliterated
        
        final_transliteration = " ".join(transliterated_parts)
        final_transliteration = re.sub(r'\s+', ' ', final_transliteration)
        final_transliteration = final_transliteration.strip()
        final_transliteration = self._fix_comma_spacing(final_transliteration)
        
        return final_transliteration, display_map
...
def main():
    ...
    for i, sentence in enumerate(input_texts):
        ...
        transliterated_sentence, details = transliterator.transliterate_with_ner(sentence)
        end_time = time.time()
